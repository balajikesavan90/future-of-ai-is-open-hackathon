...
from arctic_analytics.core.data_analyst import *
from arctic_analytics.streamlit.widgets.prompt_guide import render_data_analyst_prompt_guide
from arctic_analytics.core.security import SecurityError, safely_execute_code

def render_data_analyst():
    logging.info(f'render_data_analyst - {st.session_state["session_id"]}')
    st.divider()
    st.info('Arctic Analytics has access to the metadata of the files you uploaded. It will use that context to generate inspectable analysis code.')
    st.info('Generated code is executed with constrained Python validation. The results are displayed here for human review.')
