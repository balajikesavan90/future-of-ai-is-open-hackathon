...
def load_analysis_trace(raw_bytes: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        errors = sorted(
            Draft202012Validator(IMPORT_TRACE_SCHEMA).iter_errors(trace),
...
def _is_supported_resumed_message(message: Any) -> bool:
    ...
    if message_type == "function_call_output":
        return _valid_call_id(message.get("call_id")) and _has_supported_tool_output(message.get("output"))
    return False
...
def _has_supported_tool_output(output: Any) -> bool:
    """Accept only output shapes generated by this application's tools."""
    if isinstance(output, str):
        return True
    if isinstance(output, dict):
        return output.get("type") == "image_base64"
    return (
        ...
        and all(
            ...
            and isinstance(item.get("image_url"), (str, dict))
            and (
                isinstance(item["image_url"], str)
                or item["image_url"].get("type") == "image_base64"
            )
            for item in output
        )
    )
...
def _match_manifest_datasets(datasets: list[Any], uploaded: dict[str, dict[str, Any]]) -> dict[str, str]:
    ...
    for item in datasets:
        ...
        if (
            not _is_safe_dataset_key(trace_key)
...
def _is_safe_dataset_key(value: Any) -> bool:
    return (
        isinstance(value, str)
        and value.isidentifier()
        and not keyword.iskeyword(value)
        and value not in RESERVED_EXECUTION_GLOBAL_NAMES
    )
